<template>
    <div>
        ...
        <VCard title="Update Particular Function Details">
...
export default {
    ...
    methods: {
        async fetchData() {
            this.loader = true;
            await http
                .get("/particular-function/show/" + this.paramsId)
                .then((res) => {
                    if (res.data.success) {
                        const resData = res.data.data;
                        this.insertData.name = resData.name;
                        this.insertData.is_multiple_days = resData.is_multiple_days;
                    }
                })
                .catch((e) => {
                    console.log(e);
        ...
        async updateData() {
            ...
            if (checkValidation.valid) {
                this.loader = true;
                http
                    .post("particular-function/update/" + this.paramsId, this.insertData)
                    .then((res) => {
                        if (res.data.success) {
