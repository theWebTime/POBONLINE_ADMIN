    ...
    <div>
        ...
        <VCard>
            <VCardText class="d-flex flex-wrap py-4 gap-4">
                ...
                <div class="app-user-search-filter d-flex align-center flex-wrap gap-4">
                    <router-link to="/particularFunction/add">
                        <VBtn prepend-icon="tabler-plus"> Add Particular Function </VBtn>
                    </router-link>
                </div>
            </VCardText>
            <VDivider />
                ...
                <v-data-table class="text-no-wrap" fixed-header>
                    ...
                    <tbody>
                        <tr v-for="(item, index) in data.data" :key="index">
                            ...
                            <td class="text-center">
                                {{ item.is_multiple_days == 1 ? "Multiple Days" : "Single Day" }}
                            </td>
                            <td class="text-center">
                                <router-link :to="'/particularFunction/editParticularFunction/' + item.id">
                                    <IconBtn>
...
export default {
    ...
    methods: {
        ...
        fetchData() {
            ...
            http
                .get(
                    "/particular-function/index?page=" +
                    this.options.page +
                    "&itemsPerPage=" +
        ...
        deleteData() {
            http
                .post("/particular-function/delete/" + this.editableId, {})
                .then((res) => {
                    if (res.data.success) {
                        this.fetchData();
