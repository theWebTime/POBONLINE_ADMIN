      ...
      <VCard>
        <VCardText class="d-flex flex-wrap py-4 gap-4">
          ...
          <router-link to="/category/add">
            <VBtn prepend-icon="tabler-plus"> Add Category </VBtn>
          </router-link>
        </div>
        </VCardText>
        <VDivider />
          ...
          <v-data-table class="text-no-wrap" fixed-header>
            <thead>
              <tr>
                ...
                <th class="text-uppercase text-center">Category Price</th>
            ...
            <tbody>
              <tr v-for="(item, index) in data.data" :key="index">
                ...
                <td class="text-center">
                  <router-link :to="'/category/editCategory/' + item.id">
                    <IconBtn>
...
import http from "../../http-common";
  export default {
    ...
    methods: {
      ...
      deleteData() {
        http
          .post("/category/delete/" + this.editableId, {})
          .then((res) => {
            if (res.data.success) {
              this.fetchData();
